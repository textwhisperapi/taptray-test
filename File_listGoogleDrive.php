...
function mime_to_audio_ext($mime): string {
...
function mime_to_google_ext($mime): string {
  switch (strtolower((string)$mime)) {
...
function normalize_drive_items(array $items): array {
  foreach ($items as &$item) {
    ...
    if (strpos($mimeType, 'application/vnd.google-apps.') === 0 && !preg_match('/\\.[a-z0-9]+$/i', $name)) {
      $ext = mime_to_google_ext($mimeType);
      if ($ext !== '') {
        $name .= '.' . $ext;
      }
    }
...
if ($folderId === 'root') {
  ...
  $items = normalize_drive_items($items);
