...
$engineRequested = strtolower(trim((string)($_POST['engine'] ?? 'default')));
if (!in_array($engineRequested, ['default', 'audiveris', 'homr'], true)) {
    $engineRequested = 'default';
}
...
// OMR backend hooks:
// - OMR_COMMAND (default)
// - OMR_COMMAND_AUDIVERIS
// - OMR_COMMAND_HOMR
// All commands may use {input} and {outdir} placeholders.
$defaultTemplate = trim((string)(getenv('OMR_COMMAND') ?: ''));
$audiverisTemplate = trim((string)(getenv('OMR_COMMAND_AUDIVERIS') ?: ''));
$homrTemplate = trim((string)(getenv('OMR_COMMAND_HOMR') ?: ''));
if ($audiverisTemplate === '' && is_executable('/opt/audiveris/bin/Audiveris')) {
    $audiverisTemplate = '/opt/audiveris/bin/Audiveris -batch -transcribe -export -output {outdir} {input}';
}
if ($defaultTemplate === '' && $audiverisTemplate !== '') {
    $defaultTemplate = $audiverisTemplate;
}

$cmdTemplate = '';
$engineUsed = $engineRequested;
if ($engineRequested === 'audiveris') {
    $cmdTemplate = $audiverisTemplate !== '' ? $audiverisTemplate : $defaultTemplate;
    $engineUsed = $audiverisTemplate !== '' ? 'audiveris' : 'default';
} elseif ($engineRequested === 'homr') {
    $cmdTemplate = $homrTemplate;
    $engineUsed = 'homr';
} else {
    $cmdTemplate = $defaultTemplate;
    if ($defaultTemplate !== '') {
        if ($homrTemplate !== '' && $defaultTemplate === $homrTemplate) $engineUsed = 'homr';
        elseif ($audiverisTemplate !== '' && $defaultTemplate === $audiverisTemplate) $engineUsed = 'audiveris';
        else $engineUsed = 'default';
    }
}
if ($cmdTemplate === '') {
    if ($engineRequested === 'homr') {
        json_error('HOMR backend not configured. Set OMR_COMMAND_HOMR.', 501);
    }
    if ($engineRequested === 'audiveris') {
        json_error('Audiveris backend not configured. Set OMR_COMMAND_AUDIVERIS or install Audiveris.', 501);
    }
    json_error('OMR backend not configured. Set OMR_COMMAND (or OMR_COMMAND_AUDIVERIS / OMR_COMMAND_HOMR).', 501);
}
