logStep("JSDrawingPDF.js repurposed for TapTray products workspace");
...
(function () {
  ...
  function updateImagePreview(url, title = "") {
    ...
    mediaEl.innerHTML = `<img src="${esc(next)}" alt="${esc(title || "Menu item")}" style="width:100%;height:100%;object-fit:cover;display:block;">`;
  }
  ...
  function renderSelectedItemWorkspace() {
    ...
    const imageHtml = parsed.image
      ? `<img src="${esc(parsed.image)}" alt="${esc(selectedTitle)}" style="width:100%;height:100%;object-fit:cover;display:block;">`
      : `<div class="tt-item-media-placeholder">Food image preview</div>`;

    host.innerHTML = `
      <div class="tt-item-details">
        <div class="tt-item-shell">
          <div id="ttItemMedia" class="tt-item-media">${imageHtml}</div>
          <div class="tt-item-main">
            <div class="tt-item-header">
              <div>
                ...
                <button id="ttItemPreviewBtn" class="tt-item-preview-btn" type="button">Preview</button>
              </div>
              <div class="tt-item-price-wrap">
                <label for="ttItemPrice">Price</label>
                <input id="ttItemPrice" class="tt-input" type="text" placeholder="e.g. 3490 ISK" value="${esc(parsed.price)}">
              </div>
            </div>
            <div class="tt-item-grid">
              <div class="tt-field">
                <label for="ttItemShortDescription">Short description</label>
                <textarea id="ttItemShortDescription" class="tt-textarea" rows="3" placeholder="Short customer-facing description">${esc(parsed.description)}</textarea>
              </div>
              <div class="tt-field">
                <label for="ttItemDetailedDescription">Detailed description</label>
                <textarea id="ttItemDetailedDescription" class="tt-textarea" rows="5" placeholder="Expanded customer-facing description">${esc(parsed.detailedDescription || "")}</textarea>
              </div>
              <div class="tt-field">
                <label for="ttItemImage">Food image URL</label>
                <input id="ttItemImage" class="tt-input" type="url" placeholder="https://example.com/dish.jpg" value="${esc(parsed.image)}">
              </div>
              <div class="tt-field">
                <label for="ttItemAllergens">Allergens</label>
                <input id="ttItemAllergens" class="tt-input" type="text" placeholder="e.g. dairy, nuts, shellfish" value="${esc(parsed.allergens)}">
              </div>
  ...
  function renderListWorkspace() {
    ...
    const items = Array.from(document.querySelectorAll(`.list-sub-item[data-token="${CSS.escape(token)}"]`))
      .map((node) => ({
        title: node.querySelector(".item-subject")?.textContent?.replace(/^•\s*/, "").trim() || "Untitled item",
        owner: node.querySelector(".item-owner")?.textContent?.trim() || "",
      }));

    const cards = items.length
      ? items.map((item) => `
          <article class="tt-card">
            <div class="tt-card-media">Food image</div>
            <div class="tt-card-body">
              <div class="tt-card-price">Set price</div>
              <h3>${esc(item.title)}</h3>
              <p>${esc(item.owner)}</p>
            </div>
          </article>
        `).join("")
      : `<div class="tt-empty">Select an item from the tree to edit it, or create items in this list to populate the products workspace.</div>`;

    host.innerHTML = `
      <div class="tt-item-details">
        <div class="tt-products-head">
          <div>
            <div class="tt-item-kicker">Products workspace</div>
            <h2 class="tt-products-title">${esc(listTitle)}</h2>
          </div>
        </div>
        <div class="tt-card-grid">${cards}</div>
      </div>
    `;
  }
