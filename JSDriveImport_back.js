...
function driveRenderNode(node) {
  ...
  //     node.mimeType === "application/vnd.dropbox.folder"
  //   ) 
    ...
    label.onclick = async (e) => {
      ...
      if (!node._loaded) {
        ...
        if (Array.isArray(node.children)) {
          node.children.forEach(child => ul.appendChild(driveRenderNode(child)));
          node._loaded = true;
          li.appendChild(ul);
        }
        // ✅ LAZY (Google Drive)
        ...
        //   const res = await fetch(
        //     `/File_listGoogleDrive.php?folder=${encodeURIComponent(node.id)}`,
        //     { headers: { Authorization: `Bearer ${window.driveAccessToken}` } }
        //   );
        //   if (!res.ok) return;
        ...
        //   node.children.forEach(child => ul2.appendChild(driveRenderNode(child)));
        //   li.appendChild(ul2);

        //   label.textContent = " 📂 " + (node.name || "(folder)");
        // }
        // else if (node.id !== undefined || node.path !== undefined) {
        //   label.textContent = " 📂 " + (node.name || "(loading…)");
    ...
    checkbox.onchange = () => {
      ...
      if (Array.isArray(node.children)) {
        node.children.forEach(child => {
          ...
          const isFolder = child.mimeType?.includes("folder");
